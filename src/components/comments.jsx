...
import { Form, Button, Card, ListGroup, InputGroup, Alert, Modal, Spinner } from 'react-bootstrap';
...
function Comments({ gameId }) {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      setLoading(true);
      try {
        const response = await fetch(`https://rational-adara-alian-b9c2bb13.koyeb.app/api/comments/${gameId}`);
        if (response.ok) {
  ...
  const addComment = async () => {
    ...
    try {
      const response = await fetch('https://rational-adara-alian-b9c2bb13.koyeb.app/api/comments', {
        method: 'POST',
  ...
  const deleteComment = async () => {
    ...
    try {
      const response = await fetch(`https://rational-adara-alian-b9c2bb13.koyeb.app/api/comments/${commentToDelete.id}`, {
        method: 'DELETE',
      });
  ...
  const editComment = async () => {
    ...
    try {
      const response = await fetch(`https://rational-adara-alian-b9c2bb13.koyeb.app/api/comments/${commentToEdit.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
